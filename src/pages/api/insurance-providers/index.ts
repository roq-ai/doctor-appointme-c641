...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  async function getInsuranceProviders() {
    ...
    const response = await prisma.insurance_provider
      .withAuthorization({
        roqUserId,
      ...
      .findManyPaginated({
        ...convertQueryToPrismaUtil(query, 'insurance_provider'),
        take: limit,
        skip: offset,
        ...(order?.length && {
  ...
  async function createInsuranceProvider() {
    await insuranceProviderValidationSchema.validate(req.body);
    const body = { ...req.body };

    const data = await prisma.insurance_provider.create({
      data: body,
    });
